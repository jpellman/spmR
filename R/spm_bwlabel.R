...
#' @title SPM: Connected component labelling in 2D or 3D - a compiled routine
#
#' @usage FORMAT: [L,NUM] = spm_bwlabel(bw,conn)
# 
#
# Input:
#' @param bw (Binary) image to perform labelling on. Can
#'               be 2 or 3D.
#' @param conn Connectivity criterion. Could be 6(surface)
#'               18(edge) or 26(corner). For a 2D bw these
#'               correspond to 4, 8 and 8 respectively.
#
# Output:
#' @return A list containing: L           : Connected component image, i.e. image where
#'               each non-zero voxel in bw will have a value
#'               corresponding to its label.
#' NUM         : Number of connected components in L.
#
#
#' @description The calling interface has been modelled on the 
